...
{
  ts_printf("thread %lu pid %lu tid %lu ready to run exec.\n", k, getpid(), gettid());
  char* const args[] = {
    (char* const)"cat",
...
static void* threaded(void* param) {
  ...
  if (k == 5) {
    ...
    if (pid > 0) {
      int status;
      ts_printf("after fork, I'm parent pid = %u, child pid = %u, tid = %u\n", getpid(), pid, gettid());
      waitpid(pid, &status, 0);
      ts_printf("parent pid = %u exit\n", getpid());
    } else {
      ts_printf("after fork, I'm child pid = %u, parent = %u, tid = %u\n", getpid(), getppid(), gettid());
...
{
  ts_printf("pthread_atfork parent pid = %u, ppid = %u, tid = %u.\n", getpid(), getppid(), gettid());
}

static void atfork_child(void)
{
  ts_printf("pthread_atfork child pid = %u, ppid = %u, tid = %u.\n", getpid(), getppid(), gettid());
}
