...
void bpf_patch_all(void)
{
  struct sock_filter filter [] = {
    BPF_STMT(BPF_LD+BPF_W+BPF_ABS, offsetof(struct seccomp_data, nr)),
    BPF_JUMP(BPF_JMP+BPF_JEQ+BPF_K, SYS_mmap, 10, 0),
    BPF_JUMP(BPF_JMP+BPF_JEQ+BPF_K, SYS_exit, 9, 0),
    BPF_JUMP(BPF_JMP+BPF_JEQ+BPF_K, SYS_exit_group, 8, 0),
    BPF_JUMP(BPF_JMP+BPF_JEQ+BPF_K, SYS_wait4, 7, 0),
    BPF_JUMP(BPF_JMP+BPF_JEQ+BPF_K, SYS_clone, 6, 0),
    BPF_JUMP(BPF_JMP+BPF_JEQ+BPF_K, SYS_fork, 5, 0),
    BPF_JUMP(BPF_JMP+BPF_JEQ+BPF_K, SYS_vfork, 4, 0),
