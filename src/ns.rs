...
fn proc_setpgroups_write(child_pid: unistd::Pid) -> Result<()>{
    let setgroups = PathBuf::from("/proc").join(&format!("{}", child_pid)).join(PathBuf::from("setgroups"));
    let mut file = File::create(setgroups)?;
    file.write_all(b"deny")?;
    Ok(())
}

fn update_map(starting_pid: unistd::Pid, starting_uid: unistd::Uid, starting_gid: unistd::Gid) -> Result<()> {
    let uid_map_file = PathBuf::from("/proc").join(&format!("{}", starting_pid)).join(PathBuf::from("uid_map"));
    let gid_map_file = PathBuf::from("/proc").join(&format!("{}", starting_pid)).join(PathBuf::from("gid_map"));
