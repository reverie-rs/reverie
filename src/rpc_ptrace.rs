...
pub unsafe fn rpc_call(task: &TracedTask, func: u64, args: &[u64; 6]) -> i64 {
    if let Some((top, _)) = task.rpc_stack {
        ...
        let sp = Remoteable::remote(sp_addr as *mut u64);
        ...
        let sp = Remoteable::remote(old_sp_adjusted as *mut u64);
        ...
        let syscall_helper_addr_ptr = Remoteable::remote(consts::REVERIE_LOCAL_RPC_HELPER as *mut u64);
