...
static void* threaded(void* param) {
  ...
  printf("self: %lx thread %05ld (tid=%u, pid=%u, ppid=%u, pgid=%u) enter.\n", pthread_self(), k, tid, pid, ppid, pgid);

  ptr[k] = pthread_self();

  test_clock_nanosleep(TIME_100MS);

  printf("self: %lx thread %05ld (tid=%u, pid=%u, ppid=%u, pgid=%u) exit.\n", pthread_self(), k, tid, pid, ppid, pgid);
...
static void thread_test_1(void)
{
  ...
  long i = 0;
  assert(pthread_create(&threadid[i], &attr, threaded_0, (void*)(100+i)) == 0);
  pthread_join(threadid[i], NULL);

  for (i = 1; i < NR_THREADS; i++) {
    assert(pthread_create(&threadid[i], &attr, threaded, (void*)(100+i)) == 0);
  }
