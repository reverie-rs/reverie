...
impl SyscallInfo {
    pub fn from(tid: Pid, no: SyscallNo, args: &SyscallArgs) -> Self {
        ...
        let args = match no {
            ...
            SYS_brk => vec![SyscallArg::Ptr(ptr!(u64, a0))],
            SYS_mmap => vec![
                SyscallArg::Ptr(ptr!(u64, a0)),
                SyscallArg::Hex(a1),
            ...
            SYS_munmap => {
                vec![SyscallArg::Ptr(ptr!(u64, a0)), SyscallArg::Hex(a1)]
            }
            SYS_mprotect => vec![
                SyscallArg::Ptr(ptr!(u64, a0)),
                SyscallArg::Hex(a1),
            ...
            SYS_set_robust_list => {
                vec![SyscallArg::Ptr(ptr!(u64, a0)), SyscallArg::Int(a1)]
            }
            ...
            SYS_uname => vec![SyscallArg::UnamePtr(ptr!(u64, a0))],
            SYS_access => vec![
                SyscallArg::CStr(ptr!(i8, a0)),
                SyscallArg::FdModes(a1 as i32),
            ],
            SYS_getuid | SYS_getgid | SYS_geteuid | SYS_getegid => Vec::new(),
            SYS_time => vec![SyscallArg::PtrOut(ptr!(u64, a0))],
            SYS_gettimeofday => vec![
                SyscallArg::TimevalPtr(ptr!(u64, a0)),
            ...
            SYS_nanosleep => vec![
                SyscallArg::TimespecPtr(ptr!(u64, a0)),
                SyscallArg::TimespecPtr(ptr!(u64, a1)),
            ],
            ...
            SYS_sysinfo => vec![SyscallArg::Ptr(ptr!(u64, a0))],
            SYS_execve => vec![
                SyscallArg::CStr(ptr!(i8, a0)),
        ...
        SyscallInfo {
            ...
            args: args
                .iter()
                .map(|a| InferiorSyscallArg::from(tid, *a))
                .collect(),
            nargs_before: k,
            retval: None,
        }
...
impl GuestMemoryAccess for InferiorSyscallArg {
    fn peek_bytes(&self, addr: Remoteable<u8>, size: usize) -> Result<Vec<u8>> {
        match addr {
            ...
            Remoteable::Remote(rptr) => ptrace_peek_bytes(self.pid, rptr, size),
        }
    }
...
impl InferiorSyscallArg {
    ...
    fn from_cstr_sized<'a>(&self, ptr: Remoteable<i8>, size: usize) -> CString {
        ...
        unsafe { CString::from_vec_unchecked(slice) }
    }
    #[allow(unused)]
    fn from_cstr_sized_atmost(
        &self,
        ptr: Remoteable<i8>,
        size: usize,
        max_size: usize,
    ...
    fn fmt_cstr_sized_atmost(
        ...
        let suffix = if max_size < size { "..." } else { "" };
        write!(
            f,
            "\"{}\"{}",
            escape(self.from_cstr_sized_atmost(ptr, size, max_size)),
            suffix
        )
    }
    #[allow(unused)]
    ...
    #[allow(unused)]
    pub unsafe fn fmt_cstr_null_terminated(
        &self,
        f: &mut fmt::Formatter,
        pptr: Remoteable<u64>,
    ) -> fmt::Result {
        ...
        loop {
            ...
            let z = match pptr {
                Remoteable::Local(_) => {
                    Remoteable::local(addr as *mut i8).unwrap()
                }
                Remoteable::Remote(_) => {
                    Remoteable::remote(addr as *mut i8).unwrap()
                }
            };
    ...
    fn fmt_dirent_ptr(
        ...
        while left > 0 {
            ...
            curr = Remoteable::remote(
                (curr.as_ptr() as u64 + ent.d_reclen as u64)
                    as *mut linux_dirent_partial,
            )
            .unwrap();
        }
    ...
    fn fmt_rt_sigaction_p(
        &self,
        f: &mut fmt::Formatter,
        ptr: Remoteable<kernel_sigaction>,
    ) -> fmt::Result {
...
impl Display for InferiorSyscallArg {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self.arg {
            ...
            SyscallArg::Ptr(p_) | SyscallArg::PtrOut(p_) => match p_ {
                None => write!(f, "NULL"),
                Some(p) => write!(f, "{:x?}", p),
            },
            ...
            | SyscallArg::CStrArrayNulTerminated(None) => write!(f, "NULL"),
            SyscallArg::CStr(Some(ptr)) => self.fmt_cstr(f, ptr),
            ...
            SyscallArg::TimevalPtr(tv_) => match tv_ {
                None => write!(f, "NULL"),
                Some(tv) => self.fmt_timeval_p(f, tv.cast()),
            },

            SyscallArg::TimespecPtr(tp_) => match tp_ {
                None => write!(f, "NULL"),
                Some(tp) => self.fmt_timespec_p(f, tp.cast()),
            },
            SyscallArg::TimezonePtr(tz_) => match tz_ {
                None => write!(f, "NULL"),
            ...
            SyscallArg::Fcntl(cmd, arg) => fmt_fcntl(self.pid, cmd, arg, f),
            SyscallArg::Ioctl(cmd, arg) => fmt_ioctl(self.pid, cmd, arg, f),
            SyscallArg::UnamePtr(Some(ptr)) => self.fmt_uname_ptr(f, ptr),
            SyscallArg::CStrArrayNulTerminated(Some(ptr)) => unsafe {
                self.fmt_cstr_null_terminated(f, ptr.cast())
            },
            SyscallArg::Envp(Some(ptr)) => unsafe { self.fmt_envp(f, ptr) },
...
fn fmt_pre_syscall(info: &SyscallInfo, f: &mut fmt::Formatter) -> fmt::Result {
    ...
    let args_to_format = info.args.iter().take(info.nargs_before).format(", ");
    write!(
        f,
        "[pid {:>4}] {:?}({}{}",
...
fn fmt_post_syscall(info: &SyscallInfo, f: &mut fmt::Formatter) -> fmt::Result {
    let prefix =
        if info.nargs_before == 0 || info.nargs_before == info.args.len() {
            ""
        } else {
            ","
        };
    ...
    let args_to_format = info.args.iter().skip(info.nargs_before).format(", ");
    write!(
        f,
        "{}{}{} {}",
        prefix,
        args_to_format,
        suffix,
        info.retval.unwrap_or(SyscallRet::NoReturn)
    )
}
